...
1) La UI/cliente puede enviar directamente POST a `POST /api/multi-model-chat` para usar la orquestación multi‑modelo.
   - Nota: el endpoint principal `POST /api/chat` usa el CoreOrchestrator (LangGraph-style). No hace forwarding automático a `multi-model-chat`.
2) `multi-model-chat` valida el body con Zod, normaliza opciones y compone `metadata`:
   ...
   - `threadId` (normalmente `chatId`) se usa para aislar memoria/RAG por conversación.
...
- Llama directamente al endpoint `POST /api/multi-model-chat` (este flujo no está integrado al selector de modelos del chat principal).
- Para activar RAG, envía `options.enableSearch=true` (se mapea a `metadata.useRAG`).
- Opcional: ajustar `options.temperature` y `metadata.systemPromptVariant`.
