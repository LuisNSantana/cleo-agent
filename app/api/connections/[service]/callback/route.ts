...
export async function GET(
  ...
  try {
    ...
    if (!supabase) {
      return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}?error=database_error`)
    }
    ...
    switch (service) {
      ...
      case "google-drive":
        tokenData = await exchangeGoogleCode(code, service)
        ...
        break
      case "notion":
        tokenData = await exchangeNotionCode(code)
        accountInfo = await getNotionUserInfo(tokenData.access_token)
        break
      default:
        return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}?error=unsupported_service`)
    }
        ...
        <body>
          <script>
            if (window.opener) {
              ...
              window.location.href = '${process.env.NEXT_PUBLIC_APP_URL}?error=callback_failed';
            }
          </script>
...
async function exchangeGoogleCode(code: string, service: string): Promise<any> {
